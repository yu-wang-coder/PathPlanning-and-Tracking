...
class C:  # 存储路径规划中使用的各种参数和常量
    PI = math.pi

    XY_RESO = 2.0  # 路径规划中的坐标分辨率，即离散化网格的大小，单位为米
    YAW_RESO = np.deg2rad(15.0)  # 航向角分辨率，以弧度（rad）表示，用于定义车辆转向角的精度
    MOVE_STEP = 0.4  # [m] 路径插值分辨率，定义了路径插值时每步移动的距离，单位为米
    N_STEER = 20.0  # 转向命令数量，用于定义车辆转向时可选择的不同转向角度的数量
    COLLISION_CHECK_STEP = 5  # 碰撞检查时跳过的步数，用于加速碰撞检测过程
    EXTEND_BOUND = 1  # 碰撞检查范围的扩展距离，用于扩展车辆周围的碰撞检查范围

    GEAR_COST = 100.0  # 切换回退的惩罚成本，表示在路径规划中切换方向时的额外成本
    BACKWARD_COST = 5.0  # 后退的惩罚成本，表示车辆后退时的成本
    STEER_CHANGE_COST = 5.0  # 转向角度变化的惩罚成本，用于惩罚车辆转向角度变化过大的情况
    STEER_ANGLE_COST = 1.0  # 转向角度的惩罚成本，用于惩罚车辆转向角度过大的情况
    H_COST = 15.0  # 启发式成本的惩罚成本，用于启发式搜索中的额外成本

    RF = 4.5  # [m] 车辆后端到车辆前端的距离，单位为米
    RB = 1.0  # [m] 车辆后端到车辆后端的距离，单位为米
    W = 3.0  # [m] 车辆的宽度，单位为米
    WD = 0.7 * W  # [m] 左右轮之间的距离，为车辆宽度的百分之七十
    WB = 3.5  # [m] 车轴距，即车辆前后轮之间的距离，单位为米
    TR = 0.5  # [m] 轮胎半径，单位为米
    TW = 1  # [m] 轮胎宽度，单位为米
    MAX_STEER = 0.6  # [rad] 最大转向角度，用弧度（rad）表示，表示车辆允许的最大转向角度

class Node: # 节点表示类，包括位置、角度等信息
...
def extract_path(closed, ngoal, nstart): # 输出路径
    rx, ry, ryaw, direc = [], [], [], []
    cost = 0.0
    node = ngoal
...
def calc_next_node(n_curr, c_id, u, d, P): # 计算写一个节点
    step = C.XY_RESO * 2
...
def is_index_ok(xind, yind, xlist, ylist, yawlist, P): # 判断是否碰到障碍物
    if xind <= P.minx or \
            xind >= P.maxx or \
            yind <= P.miny or \
            yind >= P.maxy:
...
def update_node_with_analystic_expantion(n_curr, ngoal, P): # 更新节点信息
...
def is_collision(x, y, yaw, P): # 障碍物判断
...
def calc_rs_path_cost(rspath): # 计算路径代价
    cost = 0.0
...
def calc_hybrid_cost(node, hmap, P): # 计算混合代价
    cost = node.cost + \
...
def calc_parameters(ox, oy, xyreso, yawreso, kdtree): # 计算一些限制参数
...
def draw_car(x, y, yaw, steer, color='black'): # 绘制小车模型
...
def design_obstacles(x, y): # 设置地图障碍物
    ox, oy = [], []
...
def main():
    ...
    x, y = 51, 31
    sx, sy, syaw0 = 10.0, 7.0, np.deg2rad(5.0)
