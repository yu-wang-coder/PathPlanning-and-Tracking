...
class Gear(Enum): # 定义驾驶模式
    GEAR_DRIVE = 1
    GEAR_REVERSE = 2


class VehicleState: # 表示车辆状态
    ...
    def UpdateVehicleState(self, delta, a, e_cg, theta_e,
                           gear=Gear.GEAR_DRIVE): # 根据输入更新车辆的状态，包括位置、方向、速度和转向角
        """
...
class TrajectoryAnalyzer: # 分析车辆相对于给定轨迹（x、y、yaw、k）的位置
    ...
    def ToTrajectoryFrame(self, vehicle_state): # 计算车辆相对于轨迹的侧向误差（e_cg）和角度误差（theta_e），并更新车辆的位置跟踪误差
        """
...
class LatController: # 使用LQR（线性二次调节器）实现横向控制
    """
    Lateral Controller using LQR
    """

    def ComputeControlCommand(self, vehicle_state, ref_trajectory): # 根据当前车辆状态和参考轨迹计算最优转向角度（steer_angle），包括反馈和前馈控制
        """
...
def generate_path(s): # 使用Reed-Shepp路径生成器在指定的路点之间创建平滑路径
    """
