...
# 生成所有可能的路径，并对每条路径进行局部路径规划，将全局坐标系转换为局部坐标系。
...
# 生成局部路径，根据不同的路径类型（直线、转弯等）插值生成连续的路径点
...
# 根据路径类型和长度，在两个路径点之间进行插值，生成连续的路径点
